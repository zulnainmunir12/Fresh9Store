...
import 'package:fresh9_rider/core/viewmodel/featured_products_view_model.dart';
...
class FeaturedProductsView extends StatefulWidget {
  String index;
  FeaturedProductsView(this.index);
  _Categories createState() => _Categories();
}

class _Categories extends State<FeaturedProductsView> {
  ...
  Widget build(BuildContext context) {
    return ViewModelBuilder.reactive(
        viewModelBuilder: () => FeaturedProductsViewModel(),
        onModelReady: (model) => onModelReady(model),
        builder: (context, FeaturedProductsViewModel model, child) {
          return Scaffold(
              backgroundColor: AppColor.lightGrey,
              appBar: customAppBarWithTitle('Featured Products', model.count, model),
  ...
  onModelReady(FeaturedProductsViewModel model) {
    model.getProducts(widget.index);
  }

  body(FeaturedProductsViewModel model) {
    return model.products.length == 0
        ...
        : ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return _categoryCard(model.featuredProducts[index], model);
            },
          );
  }

  _categoryCard(Product product, FeaturedProductsViewModel model) {
    return Stack(
      children: [
        Container(
...
//                      )
  ...
  _quantityButton(FeaturedProductsViewModel model, Product product) {
    return Container(
      height: 40,
      width: 130,
  ...
  _addToCartButton(FeaturedProductsViewModel model, Product product) {
    return Container(
      height: 40,
      width: 130,
      padding: EdgeInsets.symmetric(horizontal: 10),
